...
ren = entsoe.client.query_wind_and_solar_forecast("NL", start=start-pd.DateOffset(months=1), end=end, psr_type=None).sum(axis=1)
gen = entsoe.client.query_generation("NL", start=start-pd.DateOffset(months=1), end=end)
load = entsoe.client.query_load_forecast("NL", start=start-pd.DateOffset(months=1), end=end)
...
df = pd.concat([DAH_NL, gen_NG, gen_coal, gen_ren, ren, load], axis=1).dropna()
...
df = df.dropna()
df.columns = ["PW_NL_DAH", "gen_NG_D+1", "gen_coal_D+1", "gen_ren_D+1", "ren", "load", "TTF", "Coal", "EUA"]
